...
const char * const BITCOIN_CONF_FILENAME = "carpaticoin.conf";
...
#else
    const char* pszModule = "carpaticoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Carpaticoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Carpaticoin";
#else
    // Unix
    return pathRet / ".carpaticoin";
#endif
#endif
}
