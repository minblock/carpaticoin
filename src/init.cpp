 ...
 */
static const char* BITCOIN_PID_FILENAME = "carpaticoind.pid";
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    RenameThread("carpaticoin-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/carpaticoin-project/carpaticoin>";
    const std::string URL_WEBSITE = "<https://carpaticoin.org>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("carpaticoin-loadblk");
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if carpaticoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if carpaticoin is started while in a temporary directory.\n",
