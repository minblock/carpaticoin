Carpaticoin Core version 0.18.1 is now available from:

  <<https://download.carpaticoin.org/carpaticoin-0.18.1/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://groups.google.com/forum/#!forum/carpaticoin-dev>
...
`/Applications/Carpaticoin-Qt` (on Mac) or `carpaticoind`/`carpaticoin-qt` (on
Linux).
...
Carpaticoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Carpaticoin Core on unsupported systems.

Carpaticoin Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Carpaticoin Core does not yet change appearance when
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/carpaticoin-project/carpaticoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@carpaticoin.org) the Carpaticoin Core
...
- A new short [document](https://github.com/carpaticoin-project/carpaticoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/carpaticoin-project/carpaticoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/carpaticoin-project/carpaticoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/carpaticoin-project/carpaticoin/blob/master/doc/carpaticoin-conf.md)
  about the `carpaticoin.conf` file describes how to use it to configure
  Carpaticoin Core.

- A new document introduces Carpaticoin Core's BIP174 [Partially-Signed
  Carpaticoin Transactions
  (PSBT)](https://github.com/carpaticoin-project/carpaticoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/carpaticoin-project/carpaticoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Carpaticoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Carpaticoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/carpaticoin-project/carpaticoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/carpaticoin-project/carpaticoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Carpaticoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Carpaticoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/carpaticoin-project/carpaticoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/carpaticoin-project/carpaticoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart carpaticoind with the `-deprecatedrpc=generate`
...
- A new `carpaticoin-wallet` tool is now distributed alongside Carpaticoin
...
This section describes planned changes to Carpaticoin Core that may affect
other Carpaticoin software and services.

- Since version 0.16.0, Carpaticoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Carpaticoin Core 0.20 (expected about
  a year after 0.18), Carpaticoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Carpaticoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Carpaticoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Carpaticoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Carpaticoin Core
...
- A sub-project of Carpaticoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Carpaticoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Carpaticoin Core's own implementation, although entropy
  gathered by Carpaticoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Carpaticoin Core a
...
- On macOS, Carpaticoin Core now opts out of application CPU throttling
